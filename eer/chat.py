...
class ChatWindow:
    def __init__(self, username, client):
        ...
        # --- Header ---
        header = tk.Label(self.root, text="GROUP NAME",
                          font=("Arial", 18, "bold"),
        ...
        send_btn = tk.Button(bottom_frame, text="Send", font=("Arial", 12, "bold"),
                             bg="green", fg="white", command=self.send_message)
    ...
    def add_message(self, name, msg, is_me=False):
        ...
        tk.Label(block, text=name, font=("Arial", 8, "bold"),
                 bg="white", fg=name_fg).pack(anchor=name_anchor)

        bubble = tk.Label(block, text=msg, bg=bubble_bg,
                          font=("Arial", 12), wraplength=300,
                          justify="left", padx=10, pady=6,
                          bd=1, relief="solid")
        bubble.pack(anchor=bubble_anchor, pady=(2, 0))

        tk.Label(block, text=timestamp, font=("Arial", 7),
                 fg="gray", bg="white").pack(anchor=bubble_anchor, pady=(2, 0))
    ...
    def receive_messages(self):
        while True:
            try:
                ...
                if not msg:
                    break
                if msg.startswith(self.username + ":"):  # skip own (already shown)
                    continue
